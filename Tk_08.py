...
# ═══════════════════════════════════════════════════════════════
# HILO SERIAL (CORREGIDO PARA ESP32)
# ═══════════════════════════════════════════════════════════════
class HiloSerial(threading.Thread):
    ...
    def conectar(self, pto, baud):
        try:
            ...
            self.ser.timeout = 2
            # Apagamos DTR y RTS para evitar que el ESP32 se reinicie al conectar
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                if raw.startswith('N,temp'): continue # Ignorar encabezado si llega
                ...
                if len(p) >= 5:
                    ...
                    self.m.q_log.put(('ERROR', f'ESP32 dice: {raw}'))
            except Exception as e:
                self.m.q_log.put(('ERROR', f'Error de lectura: {str(e)}'))
...
if __name__ == '__main__':
    Dashboard().run()#!/usr/bin/env python3
"""
Lab Tk-8 — Dashboard HMI Completo
Integra: Tkinter dark UI + matplotlib + pyserial + threading + CSV
MCU envía: 'N,temp,pres,hum,alt\n'  (CSV por UART)
